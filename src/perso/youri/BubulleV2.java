...
public class BubulleV2{
	
	private Graph bulles;
	private int numTrajectoire;
	private ArrayList<String> alTrajectoire;
	
	public BubulleV2(Graph graph) throws JSONException, IOException {
		bulles = graph;
		numTrajectoire = 0;
		alTrajectoire = new ArrayList<String>();
		
		JSONObject jobj = new JSONObject(ProcessUtils.fileString("conf/configuration2.json"));
		double minSeuilDistance = jobj.getJSONObject("seuilDistance").getDouble("min");
		double maxSeuilDistance = jobj.getJSONObject("seuilDistance").getDouble("max");
		double stepSeuilDistance = jobj.getJSONObject("seuilDistance").getDouble("pas");

		double minSeuilErreur = jobj.getJSONObject("seuilErreur").getDouble("min");
		double maxSeuilErreur = jobj.getJSONObject("seuilErreur").getDouble("max");
		double stepSeuilErreur = jobj.getJSONObject("seuilErreur").getDouble("pas");

		double minSeuilAngle = jobj.getJSONObject("seuilAngle").getDouble("min");
		double maxSeuilAngle = jobj.getJSONObject("seuilAngle").getDouble("max");
		double stepSeuilAngle = jobj.getJSONObject("seuilAngle").getDouble("pas");
		double seuilAngle = minSeuilAngle;
		
		for(double seuilDistance = minSeuilDistance; seuilDistance <= maxSeuilDistance; seuilDistance += stepSeuilDistance){
			for(double seuilErreur = minSeuilErreur; seuilErreur <= maxSeuilErreur; seuilErreur += stepSeuilErreur){
				if(seuilErreur == maxSeuilErreur){
					for(seuilAngle = minSeuilAngle; seuilAngle <= maxSeuilAngle; seuilAngle += stepSeuilAngle){
						algo(bulles,seuilDistance,seuilErreur,seuilAngle,false);
						algo(bulles,seuilDistance,seuilErreur,seuilAngle,true);
					}
				} else {	
					algo(bulles,seuilDistance,seuilErreur,seuilAngle,false);
					algo(bulles,seuilDistance,seuilErreur,seuilAngle,true);
	...
	/** Paramètres
		* @param graph 			: Graphe représentant le fichier de point dont les arrètes ont Ã©tÃ© prÃ©alablement crÃ©e
